Vagrant.configure("2") do |config|
    ...
    config.vm.network "private_network", ip: "192.168.69.101", virtualbox__intnet: "intnet"
    ...
    config.vm.provision "shell", inline: <<-SHELL
        ...
        mysql -u root -p1234 -e "CREATE USER 'grafana'@'172.20.61.89';"
        mysql -u root -p1234 -e "CREATE USER 'app'@'localhost';"

        mysql -u root -p1234 -e "GRANT ALL PRIVILEGES ON test.* TO 'app'@'localhost';"
        mysql -u root -p1234 -e "GRANT SELECT ON test.* TO 'grafana'@'172.20.61.89';"
    ...
    config.vm.provision "shell", inline: <<-SHELL
        apt install net-tools 2
        sudo ip route add default via 192.168.69.254
